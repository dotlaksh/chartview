...
if selected_table:
    ...
    stock = stocks_df.iloc[0]  # Show only the first stock for simplicity in a mobile-first view

    chart_data = fetch_stock_data(stock['symbol'], period=TIME_PERIODS['YTD'], interval=INTERVALS['Daily'])
    create_chart(chart_data.reset_index(), stock['symbol'], chart_data['Close'].iloc[-1], daily_change=0)
